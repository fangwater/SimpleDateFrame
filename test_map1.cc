...
public:
    ...
    Col<T> operator=(Col<T>& _c){
        return Col<T>(_c.values);
    }
...
class Uninit{};
using col_var = std::variant<Col<int>,Col<double>,Col<std::string>,Uninit>;
class DataFrame{
public:
...
public:
    ...
    std::any& operator()(std::string const& key, Basic_type col_type);
};

std::any& DataFrame::operator()(std::string const& key, Basic_type col_type){
    //find key from map
...
int main()
{
    ...
    auto vv = df.col<double>("tradv").values;
    ...
    df("test_concat",Basic_type::double64) = Col<std::string>(ss) + Col<std::string>(tt);
    show_v<double>(vv);
